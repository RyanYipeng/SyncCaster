...
async function getHljsStyles(): Promise<string> {
  // First check for inline styles (used in Chrome extension environment)
  const hljsInline = document.querySelector(`#hljs-inline`) as HTMLStyleElement
  if (hljsInline && hljsInline.textContent) {
    return `<style>${hljsInline.textContent}</style>`
  }

  // Fall back to external link (used in non-extension environment)
  ...
  const katexStyles = Array.from(document.styleSheets).find(sheet => {
    try {
      ...
      if (rules && rules.length > 0) {
        ...
        if (firstRule.cssText && firstRule.cssText.includes('.katex')) {
          return true
        }
      }
    } catch (e) {
...
async function getStylesToAdd(): Promise<string> {
  ...
  return [themeStyles, hljsStyles, katexStyles].filter(Boolean).join(``)
}
