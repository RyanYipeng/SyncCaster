...
watch(output, () => {
  nextTick(async () => {
    ...
    if (outputElement) {
      ...
      // In extension environment, use KaTeX (MathJax uses eval which is blocked by CSP)
      // In non-extension environment, use MathJax if available
      const mathReady = await ensureMathJax()
      if (mathReady) {
        if (isExtensionEnvironment()) {
          // Use KaTeX for extension environment
          renderPendingMathWithKaTeX(outputElement)
        } else {
          // Use MathJax for non-extension environment
          renderPendingMath(outputElement)
        }
      }
    }
  })
...
// 监听 SyncCaster 扩展的编辑器更新事件
onMounted(() => {
  window.addEventListener('synccaster-editor-update', ((event: CustomEvent<{ content: string }>) => {
    const { content } = event.detail
    if (codeMirrorView.value && content !== undefined) {
      codeMirrorView.value.dispatch({
        changes: {
          from: 0,
          to: codeMirrorView.value.state.doc.length,
